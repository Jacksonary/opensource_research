...
import com.zhaogang.other.cat.exception.CatCommonException;
...
public class CatLogbackAppender extends AppenderBase<ILoggingEvent> {
	...
	private void logError(ILoggingEvent event) {
		...
		// 防止有些戳货的一些不规范日志，不给 Exception，只给 message
		Cat.logError(event.getFormattedMessage(), new CatCommonException());
	}
